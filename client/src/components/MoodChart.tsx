...
export default function MoodChart({ entries }: MoodChartProps) {
  if (entries.length === 0) {
    return (
      <div className="bg-white rounded-2xl border border-[#F0E4DC] p-6 text-center">
        <p className="text-[#B08070] text-sm">Comece a registrar seu humor para ver a evolução</p>
      </div>
    );
  }
  ...
  const chartData = entries
    ...
    .map(entry => ({
      date: new Date(entry.createdAt).toLocaleDateString("pt-BR", { month: "short", day: "numeric" }),
      mood: entry.mood,
  ...
  const trend = moods.length >= 2 && moods[moods.length - 1] > moods[0] ? "↑ Melhorando" : moods[moods.length - 1] < moods[0] ? "↓ Desafiador" : "→ Estável";
  const trendColor = trend.includes("Melhorando") ? "text-green-600" : trend.includes("Desafiador") ? "text-amber-600" : "text-blue-600";
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-4 gap-2">
        <div className="bg-white rounded-xl border border-[#F0E4DC] p-3 text-center shadow-sm">
          ...
          <p className="font-serif text-lg font-bold text-[#C4856A]">{avgMood}</p>
          <p className="text-[10px] text-[#8B6E5A]">{moodEmojis[Math.round(parseFloat(avgMood))]}</p>
        </div>
        <div className="bg-white rounded-xl border border-[#F0E4DC] p-3 text-center shadow-sm">
          <p className="text-xs text-[#B08070] font-medium mb-1">Melhor</p>
          <p className="font-serif text-lg font-bold text-green-600">{maxMood}</p>
          <p className="text-[10px] text-[#8B6E5A]">{moodEmojis[maxMood]}</p>
        </div>
        <div className="bg-white rounded-xl border border-[#F0E4DC] p-3 text-center shadow-sm">
          <p className="text-xs text-[#B08070] font-medium mb-1">Desafio</p>
          <p className="font-serif text-lg font-bold text-amber-600">{minMood}</p>
          <p className="text-[10px] text-[#8B6E5A]">{moodEmojis[minMood]}</p>
        </div>
        <div className="bg-white rounded-xl border border-[#F0E4DC] p-3 text-center shadow-sm">
          <p className="text-xs text-[#B08070] font-medium mb-1">Tendência</p>
          <p className={`font-serif text-lg font-bold ${trendColor}`}>{trend.split(" ")[0]}</p>
      ...
      <div className="bg-white rounded-2xl border border-[#F0E4DC] p-6 shadow-sm">
        <p className="text-sm font-semibold text-[#2C1810] mb-4">Evolução do seu humor</p>
        <ResponsiveContainer width="100%" height={250}>
          <AreaChart data={chartData} margin={{ top: 10, right: 20, left: -20, bottom: 10 }}>
            ...
            <YAxis
              ...
              style={{ fontSize: "12px" }}
              tick={{ fill: "#B08070" }}
              label={{ value: "Humor", angle: -90, position: "insideLeft", style: { fill: "#B08070" } }}
            />
            <Tooltip
              contentStyle={{
