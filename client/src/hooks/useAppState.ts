...
export function useAppState() {
  ...
  const completeDay = useCallback((dayNumber: number, checkin: Omit<DayCheckin, "dayNumber" | "completedAt">) => {
    setState(prev => {
      ...
      const newState = {
        ...
        currentDay: Math.max(prev.currentDay, dayNumber + 1 <= 30 ? dayNumber + 1 : 30),
      };
      ...
      const newUnlocked = achievements
        .filter(a => !prev.unlockedAchievements.includes(a.id) && a.condition(progress))
        .map(a => a.id);
  ...
  const saveScannerResult = useCallback((result: Omit<ScannerResult, "completedAt">) => {
    setState(prev => {
      const scannerResult = { ...result, completedAt: new Date().toISOString() };
      const progress: AppProgress = {
        completedDays: prev.completedDays,
  ...
  const saveInitialAssessment = useCallback((assessment: Omit<InitialAssessment, "completedAt">) => {
    setState(prev => {
      const newState = {
        ...prev,
        initialAssessment: { ...assessment, completedAt: new Date().toISOString() },
      };
      const progress: AppProgress = {
      ...
      const newUnlocked = achievements
        .filter(a => !newState.unlockedAchievements.includes(a.id) && a.condition(progress))
        .map(a => a.id);
      return { ...newState, unlockedAchievements: [...newState.unlockedAchievements, ...newUnlocked] };
    });
  }, []);

  const saveFinalAssessment = useCallback((assessment: Omit<FinalAssessment, "completedAt">) => {
    setState(prev => {
      const newState = {
        ...prev,
        finalAssessment: { ...assessment, completedAt: new Date().toISOString() },
      };
      const progress: AppProgress = {
        completedDays: newState.completedDays,
        currentStreak: calculateStreak(newState.completedDays),
  ...
  const updateSettings = useCallback((settings: { reminderTime?: string; reminderEnabled?: boolean }) => {
