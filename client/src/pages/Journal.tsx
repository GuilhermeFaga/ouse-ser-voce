  ...
  return (
    <div className="space-y-6 pb-20 lg:pb-0">
      <div className="flex items-start justify-between">
        <div>
          <h1 className="font-serif text-2xl lg:text-3xl text-[#2C1810] mb-1">Diário Emocional</h1>
          <p className="text-[#8B6E5A] text-sm">{state.journalEntries.length} entradas</p>
        </div>
        <Button
      ...
      {showNew && (
        <div className="bg-white rounded-2xl border border-[#F0E4DC] p-6 shadow-sm space-y-4">
          ...
          <div>
            <p className="text-xs text-[#8B6E5A] mb-2">Como você está se sentindo?</p>
              ...
              {[1, 2, 3, 4, 5].map(v => (
                <button
                  ...
                  className={`flex-1 py-2 rounded-xl text-xl transition-all ${
                    newMood === v ? "bg-[#F5EDE8] scale-110" : "bg-[#FAF6F1] hover:bg-[#F5EDE8]"
                  }`}
                >
                  {moodEmojis[v]}
                </button>
            ...
            <p className="text-xs text-center text-[#B08070] mt-1">{moodLabels[newMood]}</p>
          </div>
          <Textarea
            value={newText}
      ...
      {sorted.length === 0 ? (
        <div className="text-center py-16">
          ...
          <p className="font-serif text-lg text-[#2C1810] mb-2">Seu diário está vazio</p>
          <p className="text-[#8B6E5A] text-sm max-w-xs mx-auto">
            Escreva sua primeira entrada. Não precisa ser perfeito — só precisa ser honesto.
          </p>
        </div>
      ) : (
...
function JournalCard({
  ...
  const promptKeys = Object.keys(entry.prompts).filter(k => entry.prompts[k]?.trim());
    ...
    >
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <p className="text-xs text-[#B08070] mb-0.5">{dateStr} · Dia {entry.dayNumber}</p>
          <p className="text-sm text-[#4A3728] line-clamp-2 leading-relaxed">{entry.text}</p>
        </div>
      ...
      {isExpanded && (
        <div className="px-5 pb-5 space-y-4 border-t border-[#F0E4DC] pt-4">
          <div className="bg-[#FAF6F1] rounded-xl p-4">
            <p className="text-sm text-[#4A3728] leading-relaxed whitespace-pre-wrap">{entry.text}</p>
          </div>
          {promptKeys.length > 0 && (
            <div className="space-y-3">
              <p className="text-xs text-[#B08070] font-medium uppercase tracking-wide">Reflexões do dia</p>
              {promptKeys.map(k => (
                <div key={k} className="bg-[#FAF6F1] rounded-xl p-3">
                  <p className="text-sm text-[#4A3728] leading-relaxed">{entry.prompts[k]}</p>
                </div>
              ))}
            </div>
          )}
