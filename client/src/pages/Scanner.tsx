      ...
      <div className="space-y-6 pb-20 lg:pb-0">
        ...
        <div className="bg-gradient-to-br from-[#F5EDE8] to-[#FAF6F1] rounded-2xl border border-[#E8D5CC] p-6">
          ...
          <h2 className="font-serif text-xl text-[#2C1810] mb-3">
            O que é o Scanner de Essência?
          </h2>
          <p className="text-[#6B4C3B] text-sm leading-relaxed mb-4">
            Um diagnóstico de 10 perguntas que avalia 5 áreas fundamentais da
            sua vida: identidade, equilíbrio emocional, autocuidado,
            relacionamentos e propósito.
          </p>
          <p className="text-[#6B4C3B] text-sm leading-relaxed mb-6">
            Ao final, você recebe um diagnóstico personalizado com seu nível de
            autoabandono, a área mais crítica e uma recomendação prática
            imediata.
          </p>
          <div className="grid grid-cols-2 gap-3 mb-6">
            {Object.entries(areaLabels).map(([key, label]) => (
              <div
                key={key}
                className={`flex items-center gap-2 px-3 py-2 rounded-xl ${areaBgColors[key]}`}
              >
                <div className={`w-2 h-2 rounded-full ${areaColors[key]}`} />
                <span className={`text-xs font-medium ${areaTextColors[key]}`}>
                  {label}
                </span>
              </div>
            ))}
  ...
  return (
    <div className="space-y-6 pb-20 lg:pb-0">
      ...
      <div>
        ...
        >
          <div
            className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-full ${areaBgColors[question.area]}`}
          >
            <div
              className={`w-2 h-2 rounded-full ${areaColors[question.area]}`}
            />
            <span
              className={`text-xs font-medium ${areaTextColors[question.area]}`}
            >
              {areaLabels[question.area]}
            </span>
          </div>
...
function ScannerResults({
  ...
  const abandonmentColor =
    results.abandonmentLevel > 60
      ? "text-red-600"
      : results.abandonmentLevel > 30
        ? "text-amber-600"
        : "text-green-600";
  ...
  return (
    <div className="space-y-6 pb-20 lg:pb-0">
      <div>
        ...
        <p className="text-[#8B6E5A] text-sm">
          {new Date(results.completedAt).toLocaleDateString("pt-BR", {
            day: "numeric",
            month: "long",
            year: "numeric",
          })}
      ...
      <div className={`rounded-2xl border p-6 ${abandonmentBg}`}>
        <div className="flex items-start gap-4">
          ...
          <div>
            <p className="text-xs font-medium text-[#8B6E5A] uppercase tracking-wide mb-1">
              Nível de Autoabandono
            </p>
            <div className="flex items-baseline gap-2 mb-2">
              <span
                className={`font-serif text-4xl font-bold ${abandonmentColor}`}
              >
                {results.abandonmentLevel}%
              </span>
              <span className={`font-semibold ${abandonmentColor}`}>
                {results.abandonmentLabel}
              </span>
            </div>
              ...
              <div className="flex justify-between items-center mb-1.5">
                <div className="flex items-center gap-2">
                  ...
                  {area === results.criticalArea && (
                    <span className="text-[10px] bg-[#C4856A] text-white px-2 py-0.5 rounded-full">
                      Crítica
                    </span>
                  )}
                </div>
      ...
      {/* Critical Area */}
      <div
        className={`rounded-2xl border p-6 ${areaBgColors[results.criticalArea]}`}
      >
        <p className="text-xs font-medium text-[#8B6E5A] uppercase tracking-wide mb-2">
          Área mais crítica
        </p>
        <div className="flex items-center gap-2 mb-3">
          <div
            className={`w-3 h-3 rounded-full ${areaColors[results.criticalArea]}`}
          />
          <h3
            className={`font-serif text-xl font-bold ${areaTextColors[results.criticalArea]}`}
          >
      ...
      {/* Recommendation */}
      <div className="bg-white rounded-2xl border border-[#F0E4DC] p-6 shadow-sm">
        <p className="text-xs font-medium text-[#B08070] uppercase tracking-wide mb-2">
